...
module.exports = {
	...
	exportTicket: async function (request, response) {
		try {
			const bookingId = request.params.id;
			const fileName = `ticket-${bookingId}.pdf`;

			// data booking berdasarkan Id
			const booking = await bookingModel.exportTicketByIdBooking(bookingId);
			const newDataBooking = [];
			booking.forEach((newData) => {
				let filterBy = newDataBooking.filter((value) => {
					return value.id === newData.id;
				});
				if (filterBy.length > 0) {
					// data dari filternya lebih dari 0
					let filterByIndex = newDataBooking.indexOf(filterBy[[0]]);
					newDataBooking[filterByIndex].seat = newDataBooking[
						filterByIndex
					].seat.concat(newData.seat);
				} else {
					if (typeof newData.seat === "string") {
						newData.seat = [newData.seat];
						newDataBooking.push(newData);
					}
				}
			});
			let newData;
			newDataBooking.map((value) => {
				newData = value;
			});
			ejs.renderFile(
							...
							.toFile(
								...
								(error, results) => {
									...
									return helperResponse.response(
										...
										{
											url: `http://${request.get("host")}/generate/${fileName}`,
										}
									);
								}
							);
